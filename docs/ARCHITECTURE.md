...
**浅色柔和 + 紫罗兰主色 + 圆角卡片 + 药丸控件 + 轻投影**（依据参考图 Hyz Office 风格定稿）。

- 令牌单一定义源：`frontend/src/style.css`（组件一律引用令牌，禁止散落硬编码色值）
- 主色 `#7C5CFF`（选中/主按钮），浅底 `#EFEAFF`（选中态/标签底）
- 背景 `#F4F4FB` + 渐变 `#EEF0FA`（柔和空气感）；卡片 `#FFFFFF` + 描边 `#E9EAF2` + 投影 `0 8px 24px rgba(79,70,229,.08)`
- 文字 `#1A1B2E` / `#6B6C82` / `#9A9BB0`；功能色成对：成功 `#22C55E`/`#E8F8EE`、错误 `#EF4444`/`#FDEAEA`、警告 `#F59E0B`/`#FDF3E3`
- 圆角：卡片 18px、输入 14px、按钮/标签 999px（药丸）
- 组件类：`.card` `.chip` `.btn-primary` `.btn-icon` `.caret`（流式光标）
